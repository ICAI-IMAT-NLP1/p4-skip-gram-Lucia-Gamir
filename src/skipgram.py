...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        # TODO
        input_vectors_unsqueeze = input_vectors.unsqueeze(1)  # (batch_size, 1, embed_size)
        output_vectors_unsqueeze = output_vectors.unsqueeze(2)  # (batch_size, embed_size, 1)
        dot_mult = torch.bmm(input_vectors_unsqueeze, output_vectors_unsqueeze).squeeze()  # (batch_size,)
        out_loss = torch.sum(torch.log(torch.sigmoid(dot_mult))) 

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        # input_vectors_unsqueeze = input_vectors.unsqueeze(1) -> (batch_size, 1, embed_size)
        noise_vectors_transpose = noise_vectors.transpose(1,2)  # (batch_size, embed_size, n_samples)
        dot_noise = torch.bmm(input_vectors_unsqueeze, noise_vectors_transpose).squeeze()  # (batch_size, n_samples)
        noise_loss = torch.sum(torch.log(torch.sigmoid(dot_noise)))
